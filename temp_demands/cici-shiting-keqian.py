...
on s.smownerid = u.id
where DATE_FORMAT(s.shiting3061,'%Y%m%d') between DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 4 DAY),'%Y%m%d')and DATE_FORMAT(DATE_SUB(NOW(),INTERVAL 0 DAY),'%Y%m%d')
order by s.shiting3061 , s.shiting3071 , s.clueregsid 
#,c.createdtime 
...
try:
    ...
    file = '%s-%s试听课前具体信息.xlsx'%(last_4_days,today)
...
#    cc_list = ['mashuguo@171xue.com']
    sub = '%s-%s试听课前具体信息，此为自动发送，请知悉。'%(last_4_days,today)
